...
import { crates, toys } from "@/lib/data";
...
export default function CratesAndToys() {
  ...
  return <div className="container page-shell">
    ...
    <section><h2>Crates</h2><DataTable label="Unbox ASMR crates" headers={["Name","Area / stage","Requirement","Cost","Possible toys","Event","Verified at","Evidence"]} rows={crateRows}/>{crateRows.length === 0 && <><div className="spacer-small"/><EmptyVerifiedState description="Capture each crate panel with its name, area, unlock requirement, cost, toy pool, and only the odds visibly displayed by the game."/></>}</section>
    <section className="section-compact"><h2>Toys</h2><DataTable label="Unbox ASMR toys" headers={["Name","Rarity","Source crate","Cash value","Interaction","Event","Index","Verified at"]} rows={toyRows}/>{toyRows.length === 0 && <><div className="spacer-small"/><EmptyVerifiedState description="Capture the toy detail or Complete Index UI showing the exact name, rarity, source, value, interaction type, event status, and index number."/></>}</section>
