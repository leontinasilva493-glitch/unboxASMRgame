...
function latestEvidence(record) {
  ...
  return {
    ...
    evidenceUrl: evidence?.sourceUrl ?? null,
    evidenceScreenshot: evidence?.screenshot ?? null,
    evidenceTimestamp: evidence?.videoTimestamp ?? null,
    evidenceVersion: evidence?.gameVersion ?? null,
    evidenceNotes: evidence?.notes ?? null,
  };
}

export function buildEvidenceHref(sourceUrl, videoTimestamp) {
  ...
  if (!/(^|\.)youtube\.com$|(^|\.)youtu\.be$/.test(url.hostname) || !videoTimestamp) return sourceUrl;
  const parts = videoTimestamp.split(":").map(Number);
